...
const run = async (lane, job) => {
  ...
  if (!provisioningProfile) {
    ...
    provisioningProfile = execSync(`xcodebuild -showBuildSettings -target ${target} -configuration ${buildConfiguration} | grep " PROVISIONING_PROFILE_SPECIFIER = "`).toString().replace('PROVISIONING_PROFILE = ', '').trim();
  }
  ...
  const cmdBuild = `xcodebuild -workspace ${appName}.xcworkspace -scheme ${scheme || appName} -sdk iphoneos  archive -archivePath ${process.cwd()}/${process.env.BLTmpProcessingDir}/${appName}.xcarchive`;

  // PROVISIONING_PROFILE="${provisioningProfile}"
  logger.info(`Archiving... ${cmdBuild}`);
  await genArchive(cmdBuild);

  const archivePath = `${process.cwd()}/${process.env.BLTmpProcessingDir}/${appName}.xcarchive`;

  logger.info(`🎉 Archive Generated! ${archivePath}`);

  logger.info('Builing Package now!');
  ...
  const ipaPath = `${process.cwd()}/${process.env.BLTmpProcessingDir}/${scheme}.ipa`;
  logger.info(`🎉 Build Generated! ${ipaPath}`);

  // Export Process ENVs
  process.env.BL_BUILD_ARCHIVE_PATH = archivePath;
  process.env.BL_BUILD_IPA_PATH = ipaPath;

  logger.table([['env.BL_BUILD_ARCHIVE_PATH', archivePath], ['env.BL_BUILD_IPA_PATH', ipaPath]], ['Variable', 'Value'], `${job.job} - Output Params`);
};
