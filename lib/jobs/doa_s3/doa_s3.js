...
const awsUtils = {};
awsUtils.putObject = (jobOptions, fileContent, key, contentType) => {
  aws.config = {
...
const run = async (lane, job) => {
  ...
  try {
    ...
    const ipa = await awsUtils.putObject(job.options, fileContent, `${s3Prefix}/package.ipa`);
    // Export Process ENVs
    ...
    logger.info(downloadS3.url);

    process.env.BL_DOA_S3_INSTALL_URL = downloadS3.url;

    logger.info(`${job.job} - Output Params`);
    logger.info(`env.BL_DOA_S3_BUILD_URL        : ${ipa.url}`);
    logger.info(`env.BL_DOA_S3_MANIFEST_URL     : ${manifestS3.url}`);
    logger.info(`env.BL_DOA_S3_INSTALL_URL      : ${downloadS3.url}`);
  } catch (error) {
    logger.error(error);
    process.exit();
  }
